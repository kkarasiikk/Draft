...
describe('monthCalendar — місяць повними тижнями', () => {
  ...
  // Клітинка місяця показує назву завдання, а не саму лише крапку: на
  // широкому екрані під числом є місце, і «щось є» там нічого не каже.
  describe('назви завдань у клітинці', () => {
    const named = (dueDate, title, over = {}) => ({ title, dueDate, done: false, ...over });

    test('день несе свої завдання, а не лише лічильник', () => {
      const m = H.monthCalendar([
        named('2026-09-10', 'Купити молоко'),
        named('2026-09-10', 'Подзвонити мамі'),
      ], SEP);
      const day = m.days.find((d) => d.date === '2026-09-10');
      expect(day.items.map((t) => t.title)).toEqual(['Купити молоко', 'Подзвонити мамі']);
    });

    test('порядок той самий, що в списку дня: невиконане, за часом, за назвою', () => {
      // Порядок тут важить більше, ніж у списку: у клітинку вміщається один
      // рядок, тож він вирішує не «що вище», а «що взагалі видно».
      const m = H.monthCalendar([
        named('2026-09-10', 'Ввечері', { dueTime: '18:00' }),
        named('2026-09-10', 'Зроблено', { done: true, dueTime: '07:00' }),
        named('2026-09-10', 'Без часу'),
        named('2026-09-10', 'Зранку', { dueTime: '09:00' }),
      ], SEP);
      const day = m.days.find((d) => d.date === '2026-09-10');
      expect(day.items.map((t) => t.title))
        .toEqual(['Зранку', 'Ввечері', 'Без часу', 'Зроблено']);
    });

    test('порожній день несе порожній список, а не undefined', () => {
      const m = H.monthCalendar([], SEP);
      expect(m.days.find((d) => d.date === '2026-09-10').items).toEqual([]);
    });

    // У смузі днів на телефоні під числом стоїть крапка: клітинка там
    // завширшки з палець, і назві в ній немає де стати. Масив назв був би
    // полем, якого ніхто не читає.
    test('смуга днів назв не несе — там для них немає місця', () => {
      const strip = H.dayStrip([named('2026-09-10', 'Купити молоко')], SEP, '2026-09-07', 7);
      expect(strip.days.find((d) => d.date === '2026-09-10').items).toEqual([]);
      // Але крапка на місці: лічильники смузі потрібні.
      expect(strip.days.find((d) => d.date === '2026-09-10').hasTasks).toBe(true);
    });
  });
