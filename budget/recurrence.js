// ---- Регулярні операції: обчислення наступної дати ----
// Серія наперед не генерується: наступна дата рахується від попередньої, коли
// про неї спитають. Для застосунку без сервера-планувальника це єдиний чесний
// варіант — інакше «щоденне» правило за рік створило б 365 документів, які ще
// й треба було б підчищати при зміні правила.
//
// Файл лежав у tasks/: там були повторювані завдання, а бюджет позичав
// обчислення, щоб не мати другої реалізації «кожні N днів». Повторення
// завдань прибрані, регулярні платежі лишились — і файл переїхав до свого
// єдиного тепер користувача.
//
// Правило (поле `recurrence` в документі операції):
...
//   anchor: 'completion' -- рахуємо від дня, коли операцію реально записали
//                           (кожні 7 днів ПІСЛЯ того, як це сталось)
//
