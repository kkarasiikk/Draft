...
(function (root) {
  ...
  //
  // Учорашнє сюди не потрапляє — так само, як завтрашнє. Невиконане
  // завдання лишається у своєму дні, а «зараз» відповідає на питання про
  // сьогодні. Інакше картка місяцями показувала б найстаріший борг замість
  // того, що справді варто зробити наступним.
  function groupOf(task, todayIso, nowHhmm) {
    if (!task.dueDate) return 4;
    if (task.dueDate !== todayIso) return null;
   ...
   * Завдання без дати потрапляють у чергу, лише якщо на сьогодні вже
   * нічого не лишилось.
   */
   ...
   * реалістичний план. Рахується виключно сьогоднішнє: людина має бачити
   * навантаження на день, а не борг за весь місяць.
   *
   * Тут була ще й перевірка за кількістю — «заплановано 4, а зазвичай
   * закриваєш 3», від особистої норми з tasks/stats.js. Її прибрано на
   * прохання: попередження спрацьовувало мало не щодня (норма — медіана,
   * тож план її перевищує в половині випадків за визначенням) і заважало
   * читати картку. Сама норма лишилась — її показує екран статистики.
   * Попередження за часом лишається: воно каже про конкретну нестачу
   * годин, а не про статистику.
   */
