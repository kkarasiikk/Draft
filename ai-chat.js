...
// ---- Голос ----
  ...
  // Рушій відкриває мікрофон сам і закриває його, щойно завершиться, — а
  // завершується він часто (див. коментар вище). Там, де дозвіл не
  // запам'ятовується, кожне таке відкриття перепитують окремо, і на одну
  // фразу виходить кілька запитів. Власний потік тримає мікрофон зайнятим
  // від першого дотику до кінця запису, тож підхоплення відбуваються мовчки.
  //
  // Але коли дозвіл уже виданий назавжди (Chrome), цей потік не потрібен і
  // навіть шкідливий: два незалежні захоплення мікрофона браузер може
  // перепитати окремо. Тому беремо його тільки там, де без нього гірше.
